...
**Final-wrap session (2026-04-18) closed M1 + M3 + L9 + L21 as code, captured H29 decision, accepted 14 findings with rationale, queued 5 for a joint decision meeting, and filed M41 as a fresh discovery.** Session shape: decision-capture-first, with a small code tail for items where a decision could ship same-session. The three named Tier-1 items from the prompt all produced outcomes: **H29** design decision locked in as option (a) — column-level encryption on sensitive JSONB subtrees, admin-JWT-gated decrypt paths plus internal consumer routes (`generate-proposal.js`), **backfill + lazy-on-write** migration, and v2-prefix rotation built in from day one (~3 implementation sessions). H29 stays 🔷 DEFERRED on implementation but is unblock-and-execute from here. **M1** closed end-to-end: all 12 active Stripe payment links tagged via the Stripe REST API (Chris provided a restricted `rk_live_...` key for in-session use, to be rotated post-session) — 2 `entity_audit`, 9 `core_marketing_system`, 1 `strategy_call` — and `stripe-webhook.js:135-143` flipped to metadata-first with amount fallback for a 30-day observation window (commit `709bf878`). Side discovery surfaced: the else-branch after `isEntityAudit` assumes CORE Marketing System, so a Strategy Call purchase with a matching slug would mis-route through the CORE onboarding cascade — filed as **M41** (narrow blast radius, queued for the joint decision meeting or a future stripe-webhook classification session). **M3** closed partial-shape after verification that zero admin UI callers write `signed_agreements` or `payments` through `action.js` (onboarding uses direct PostgREST via anon key + page-token gate; Stripe webhook uses direct `sb.mutate`; delete-client uses direct `sb.mutate`) — flipped both tables to `{ read: true, write: false, delete: false }`; `workspace_credentials` stays admin-writable per Scott's onboarding workflow (commit `02fe46ee`). Same-session code-shippable L-tier cleanup: **L9** admin URL `#audit-` fragment dropped from all 3 sites (commits `610e95c8` + `0f1cb070`); **L21** enrich-proposal UA changed to a realistic Mozilla-prefixed string (commit `d5cf4846`). All 5 new commits READY on Vercel on first build. The restricted Stripe key is pending rotation; reminder captured in the session handoff.
...
**The audit is fully wrapped on the code-only side.** Every Critical closed, every non-deferred High closed, every code-actionable Medium / Low / Nit either resolved, accepted-with-rationale, or has a dated owner. What remains is 5 distinct follow-ups, each with a concrete shape. Prioritize as makes sense:

1. **Joint Chris + Scott decision meeting** (~30 min, product-gated). Five product decisions live here:
   - **M19** — when Stripe payment-session.completed arrives after the free-tier audit email already went out, what's desired: hold-and-refund, upgrade-and-continue, refund-and-offer-manual?
   - **M24** — GSC auto-correct on 403/404: require explicit admin confirmation, add a cooling-off check that won't overwrite on transient errors, or just log the change to `activity_log` for audit?
   - **M37** — when `contact.status` flips mid-flight (lead → onboarding/active/lost) after audit submission but before email send, should pending followups cancel, pause for review, or let run?
   - **M39** — is the one-therapist-multiple-practices scenario (shared contact email) intentional business behavior? If yes, no constraint; if no, add `UNIQUE(lower(email)) WHERE email IS NOT NULL` after cleanup.
   - **M41** (optional, newly filed) — should the stripe-webhook else-branch fail-loud on unknown `metadata.product`, or silently no-op? Closely adjacent to Strategy Call handling.
   After this meeting, each decision produces a small code commit (or a 🔷 ACCEPT-with-rationale doc commit).

2. **H29 implementation session** (~3 focused sessions of work, unblocked). Design is fully captured: column-level encryption on `proposals.enrichment_data` sensitive subtrees; admin-JWT-gated + internal-consumer read paths; backfill + lazy-on-write migration; v2-prefix rotation from day one. Breakdown: (1) extend `_lib/crypto.js` `encryptFields` to JSONB subtree targets + v2-prefix path, (2) wire write-path in `enrich-proposal.js` + decrypt-path in `generate-proposal.js` + extend action.js `SENSITIVE_FIELDS` for the column, (3) backfill migration + staging verification + deploy + lazy-on-write test.

3. **M23 single-session env-var sweep** (~30 min). `DRIVE_CLIENTS_FOLDER_ID` env var + module-load warning + fail-closed on absence, swap the literal at `generate-proposal.js:700`, re-test Drive-folder-create path for one test client. Clean, bounded, no blockers.

4. **Shared JSON-parser helper session** (when worth the round trip). L11 + M25 both strip markdown fences on Claude JSON output with `.replace(/```json/g, '')` — nested fences inside string values corrupt the parse. Find-first-brace + bracket-tracking helper closes both sites in one commit. Low priority; `JSON.parse` catches currently produce useful errors when failures hit.

5. **Post-session operational reminders**:
   - **Rotate the temporary Stripe `rk_live_...` restricted API key** provided for this session's payment-link tagging. Developers → API keys in the Stripe Dashboard → revoke.
   - **Observation window for M1 amount fallback**: after ~30 days of verifying every `checkout.session.completed` event arrives with `metadata.product`, remove the amount-threshold fallback at `stripe-webhook.js:137-143`.

Items 1 + 3 together are probably one afternoon. Items 4 is a half-session whenever. Item 2 is the largest — roughly the time investment for a dedicated 3-session arc whenever encryption-at-rest becomes a priority. The audit docs read cleanly top-to-bottom: every C/H is resolved or deferred-with-design-captured, every L/N/M is resolved, accepted-with-rationale, or has a concrete next-action. The `_lib/*.js` centralizations give the project four enforceable invariants (supabase.js error contract, github.js path allowlist, zero raw GitHub writes in live route code, zero routes echo supabase/external error detail to response bodies). Defensible resting state; path forward is unblock-and-execute.
